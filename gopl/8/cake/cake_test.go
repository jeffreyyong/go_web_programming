...
func Benchmark(b *testing.B) {
	...
	// Each stop takes exactly 10ms. No buffers.
	cakeshop := defaults
	cakeshop.Work(b.N)
}
...
func BenchmarkVariableBuffers(b *testing.B) {
	// Adding channel buffers reduces delays resulting from variability
...
func BenchmarkSlowIcing(b *testing.B) {
	// Making the middle stage slower adds directly to the critical path.
	cakeshop := defaults
...
func BenchmarkSlowIcingManyIcers(b *testing.B) {
	// Adding more icing cooks reduces the cost of icing to its sequential component
	cakeshop := defaults
	cakeshop.IceTime = 50 * time.Millisecond
	cakeshop.NumIcers = 5
	cakeshop.Work(b.N)
